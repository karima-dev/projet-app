...
const initialState: EmpruntState = {
  ...
  formData: {
    ...
    moyen: "null",
    etatemprunt: null,
    livre:[{auteur:"",date:"",id:"",src:"",titre:"",description:"",ean:"",emplacement:"",nbre:0}],
  ...
  switch (action.type) {
    ...
    case ActionsTypes.SET_FORM_DATA:
      ...
      if (action.payload.name == champsValue.champnom.name) {
        formData = { ...state.formData, nameUser: action.payload.value,livre:action.payload.livre };
      ...
      if (action.payload.name == champsValue.champdate.name) {
        formData = { ...state.formData, dateRetour: action.payload.value,dateEmprunt:getDate() };
        return { ...state, formData };
      }
      else {
        formData = { ...state.formData, dateRetour: action.payload.value,dateEmprunt:getDate() };
